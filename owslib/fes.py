...
class FilterRequest(object):
    ...
    def set(self, parent=False, qtype=None, keywords=[], typenames='csw:Record', propertyname='csw:AnyText', bbox=None, identifier=None):
        ...
        # construct
        node0 = etree.Element(util.nspath_eval('ogc:Filter'))
        if parent is True:
            node0.set(util.nspath_eval('xsi:schemaLocation'), schema_location)
        ...
        if pcount > 1: # Filter should be And-ed
            node1 = etree.SubElement(node0, util.nspath_eval('ogc:And'))
        else: 
            node1 = None
        ...
        if len(keywords) > 0:
            if len(keywords) > 1: # loop multiple keywords into an Or
                if node1 is not None:
                    node3 = etree.SubElement(node1, util.nspath_eval('ogc:Or'))
                else:
                    node3 = etree.SubElement(node0, util.nspath_eval('ogc:Or'))
    ...
    def _setpropertyisequalto(self, parent, propertyname, literal, matchcase=True):
        ...
        tmp = etree.SubElement(parent, util.nspath_eval('ogc:PropertyIsEqualTo'))
        if matchcase is False:
            tmp.set('matchCase', 'false')
        etree.SubElement(tmp, util.nspath_eval('ogc:PropertyName')).text = propertyname
        etree.SubElement(tmp, util.nspath_eval('ogc:Literal')).text = literal
    ...
    def _setbbox(self, parent, bbox):
        ...
        tmp = etree.SubElement(parent, util.nspath_eval('ogc:BBOX'))
        etree.SubElement(tmp, util.nspath_eval('ogc:PropertyName')).text = 'ows:BoundingBox'
        tmp2 = etree.SubElement(tmp, util.nspath_eval('gml:Envelope'))
        etree.SubElement(tmp2, util.nspath_eval('gml:lowerCorner')).text = '%s %s' % (bbox[0], bbox[1])
        etree.SubElement(tmp2, util.nspath_eval('gml:upperCorner')).text = '%s %s' % (bbox[2], bbox[3])
    ...
    def _setpropertyislike(self, parent, propertyname, literal, wildcard='%', singlechar='_', escapechar='\\'):  
        ...
        tmp = etree.SubElement(parent, util.nspath_eval('ogc:PropertyIsLike'))
...
class FilterCapabilities(object):
    ...
    def __init__(self, elem):
        ...
        self.spatial_operands = [f.text for f in elem.findall(util.nspath_eval('ogc:Spatial_Capabilities/ogc:GeometryOperands/ogc:GeometryOperand'))]
        ...
        self.temporal_operands = [f.text for f in elem.findall(util.nspath_eval('ogc:Temporal_Capabilities/ogc:TemporalOperands/ogc:TemporalOperand'))]
        ...
        # Scalar_Capabilities
        self.scalar_comparison_operators = [f.text for f in elem.findall(util.nspath_eval('ogc:Scalar_Capabilities/ogc:ComparisonOperators/ogc:ComparisonOperator'))]
...
def setsortby(parent, propertyname, order='ASC'):
    ...
    tmp = etree.SubElement(parent, util.nspath_eval('ogc:SortBy'))
    tmp2 = etree.SubElement(tmp, util.nspath_eval('ogc:SortProperty'))
    etree.SubElement(tmp2, util.nspath_eval('ogc:PropertyName')).text = propertyname
    etree.SubElement(tmp2, util.nspath_eval('ogc:SortOrder')).text = order
