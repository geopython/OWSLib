...
class WebMapService(object):
    ...
    def _buildMetadata(self, parse_remote_metadata=False):
        ...
        self.operations = []
        for elem in self._capabilities.find('{http://www.opengis.net/wms}Capability/{http://www.opengis.net/wms}Request')[:]:
        ...
        def gather_layers(parent_elem, parent_metadata):
            layers = []
            for index, elem in enumerate(parent_elem.findall('{http://www.opengis.net/wms}Layer')):
                cm = ContentMetadata(
                    elem, parent=parent_metadata, index=index + 1, parse_remote_metadata=parse_remote_metadata)
                if cm.id:
                    if cm.id in self.contents:
                        warnings.warn(
                            'Content metadata for layer "%s" already exists. Using child layer' % cm.id)
                    layers.append(cm)
        ...
        self.exceptions = [f.text for f
                           in self._capabilities.findall('{http://www.opengis.net/wms}Capability/{http://www.opengis.net/wms}Exception/{http://www.opengis.net/wms}Format')]
    ...
    def __build_getmap_request(
        self, layers=None, styles=None, srs=None, bbox=None,
        ...
        **kwargs : extra arguments
        ...
        try:
            base_url = next((m.get('url')
                            for m in self.getOperationByName('GetMap').methods if m.get('type').lower() == method.lower()))
        except StopIteration:
            base_url = self.url

        request = self.__build_getmap_request(
            layers=layers, styles=styles, srs=srs, bbox=bbox,
    ...
    def getfeatureinfo(self, layers=None, styles=None, srs=None, bbox=None,
        ...
        try:
            base_url = next((m.get('url')
                            for m in self.getOperationByName('GetFeatureInfo').methods if m.get('type').lower() == method.lower()))
        except StopIteration:
            base_url = self.url
...
class ServiceIdentification(object):
    ...
    def __init__(self, infoset, version):
        ...
        self.type = testXMLValue(
            self._root.find('{http://www.opengis.net/wms}Name'))
        self.version = version
        self.title = testXMLValue(
            self._root.find('{http://www.opengis.net/wms}Title'))
        self.abstract = testXMLValue(
            self._root.find('{http://www.opengis.net/wms}Abstract'))
        self.keywords = extract_xml_list(
            self._root.findall('{http://www.opengis.net/wms}KeywordList/{http://www.opengis.net/wms}Keyword'))
        self.accessconstraints = testXMLValue(
            self._root.find('{http://www.opengis.net/wms}AccessConstraints'))
        self.fees = testXMLValue(
            self._root.find('{http://www.opengis.net/wms}Fees'))
...
class ServiceProvider(object):
    ...
    def __init__(self, infoset):
        ...
        if name is not None:
            ...
            self.name = None
        self.url = self._root.find(
            '{http://www.opengis.net/wms}OnlineResource').attrib.get(
                '{http://www.w3.org/1999/xlink}href', '')
        # contact metadata
        contact = self._root.find(
...
class ContentMetadata:
    ...
    def __init__(self, elem, parent=None, children=None, index=0, parse_remote_metadata=False, timeout=30):
        if elem.tag != '{http://www.opengis.net/wms}Layer':
        ...
        self.id = self.name = testXMLValue(
            elem.find('{http://www.opengis.net/wms}Name'))
        ...
        self.title = None
        title = testXMLValue(elem.find('{http://www.opengis.net/wms}Title'))
        if title is not None:
            self.title = title.strip()

        self.abstract = testXMLValue(
            elem.find('{http://www.opengis.net/wms}Abstract'))
        ...
        if sh is not None:
            if 'min' in sh.attrib and 'max' in sh.attrib:
                self.scaleHint = {
                    'min': sh.attrib['min'], 'max': sh.attrib['max']}
        ...
        # Set self.crsOptions to None if the layer (and parents) had no SRS
        # options
...
class OperationMetadata:
    ...
    def __init__(self, elem):
        ...
        for verb in elem.findall('DCPType/HTTP/*'):
            url = verb.find('OnlineResource').attrib[
                '{http://www.w3.org/1999/xlink}href']
