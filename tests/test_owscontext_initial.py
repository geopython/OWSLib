...
def test_extract_p():
    ...
    t2 = geojson.extract_p('date', t1, None)
...
def test_json_decode():
  ...
  "features": [
    ...
    result = geojson.decode_json(jsdata)
    assert result is not None
    assert result['features'][0]['properties']['date'] == "2013-11-02T15:24:24.446+12:00"


def test_decode_single_json():
    jsondata1 = open(resource_file('owc1.geojson'), 'r').read()
    result = geojson.decode_json(jsondata1)
    assert result is not None
    assert result['features'][0]['properties']['date'] == "2013-11-02T15:24:24.446+12:00"


def test_encode_json_small():
    feat = core.OwcResource(id="http://the.resource.com/id=1",
                            update_date=datetime.now(),
                            title="resource title",
                            temporal_extent="2013-11-02T15:24:24.446+12:00")
    assert feat.temporal_extent == "2013-11-02T15:24:24.446+12:00"

    t1 = feat.to_dict()
    t2 = geojson.skip_nulls(feat.to_dict())
    ...
    owc1 = core.OwcContext(id="http://ows.com/id1",
                           ...
                           time_interval_of_interest="2013-11-02T15:24:24.446+12:00",
                           resources=[])
    assert owc1.time_interval_of_interest == "2013-11-02T15:24:24.446+12:00"

    t3 = geojson.skip_nulls(owc1.to_dict())
...
def test_decode_full_json1():
    ...
    owc1 = core.OwcContext.from_json(jsondata1)
    assert owc1 is not None
    assert owc1.resources[0].temporal_extent == "2013-11-02T15:24:24.446+12:00"
    # logger.debug(owc1.to_json())
    re_owc1 = core.OwcContext.from_json(owc1.to_json())
...
def test_decode_full_json2():
    jsondata2 = open(resource_file('owc2.geojson'), 'rb').read().decode('latin1')
    owc2 = core.OwcContext.from_json(jsondata2)
    assert owc2 is not None
    # logger.debug(owc2.to_json())
    re_owc2 = core.OwcContext.from_json(owc2.to_json())
...
def test_decode_full_json3():
    ...
    assert owc3.authors[0].email == "alexander.kmoch@ut.ee"
    ...
    assert owc3.resources[0].temporal_extent == "2011-11-04T00:01:23Z/2017-12-05T17:28:56Z"
